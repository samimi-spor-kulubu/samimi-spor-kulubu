...
import {deleteBranch, toggleActive} from './actions';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 min-w-0 flex-1">
        <span
          aria-hidden="true"
          className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-brand-surface text-2xl"
        >
          {emoji ?? '🏷'}
        </span>
        <div className="min-w-0">
          ...
          {scheduleTr && (
            <p className="mt-1 text-sm text-brand-amber">{scheduleTr}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Link
          href={`/admin/branslar/${id}/duzenle`}
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Branşı sil?</DialogTitle>
              <DialogDescription>
                Bu işlem geri alınamaz. &quot;{nameTr}&quot; branşı kalıcı
                olarak silinecek.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
